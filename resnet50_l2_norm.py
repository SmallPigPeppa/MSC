...
class ResNet50_L2(LightningModule):
    def __init__(self):
        ...
        self.std = torch.tensor([0.228, 0.224, 0.225]).view(1, 3, 1, 1)
    ...
    def forward(self, imgs):
        ...
        small_imgs = (small_imgs - self.mean.to(self.device)) / self.std.to(self.device)
        mid_imgs = (mid_imgs - self.mean.to(self.device)) / self.std.to(self.device)
        large_imgs = (large_imgs - self.mean.to(self.device)) / self.std.to(self.device)
